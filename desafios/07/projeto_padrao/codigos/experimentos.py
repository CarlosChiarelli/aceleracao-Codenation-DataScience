...
class Experimentos:
    ...
    def __init__(self):
        self.modelos_testados = {'linear' : LinearRegression(),
                                  'ridge' : Ridge(), 
    ...
    def exec_experimentos(self):
        ...
        modelos = Experimentos().treinamento_modelo(X_treino, y_treino.ravel())
        ...
        for model in modelos.keys():
            ...
            print(Metricas().calcula_regressao(y_teste, Series(y_pred)), '\n')
            metrics = Metricas().calcula_regressao(y_teste, Series(y_pred))
