...
def criar_histograma(coluna, df):
    # width = tamanho do grafico
    # mark = tipo do grafico
    chart = alt.Chart(df, width=600).mark_bar().encode(
        alt.X(coluna, bin=True),
        y='count()',
        # tooltip diz qual coluna eu quero na interatividade
        tooltip=[coluna, 'count()']
    ).interactive()
    # deixa interativo
    return chart
...
def main():
    ...
    if file is not None:
        ...
        # seleciona colunas caracteres
        colunas_object = list(aux[aux['tipos'] == 'object']['colunas'])
        colunas = list(df.columns)

        # escolhe qual coluna analisar
